...
import os
import json
from default_parameters import get_ml_dsa_instances  # Use lazy import

# Get ML-DSA instances properly
ML_DSA_44, ML_DSA_65, ML_DSA_87 = get_ml_dsa_instances()
...
class TestMLDSA(unittest.TestCase):
    ...
    def generic_test_ml_dsa(self, ML_DSA, count=5):
        for _ in range(count):
            ...
            print("\n--- ML-DSA Signature Test ---")
            print(f"Message: {msg.hex()}")
            print(f"Public Key: {pk.hex()}")
            #print(f" len Public Key: {len(pk.hex())}")
